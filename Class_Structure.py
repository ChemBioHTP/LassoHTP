...
class lassoPeptide(Child):
    def __init__(self, residues, parent=None):
        ...
        self.upperPlug=[]
        self.lowerPlug=[]
        self.essentialRes=[] #acidic linker
#------------------------
    @classmethod
    def fromPDB(cls, lasso_input, input_type='file_str'):
        '''
        extract from PDB.
        ---------
        lasso_input = file_str (or path or file)
        split the file_str to residues and init each residue
        ''' 
        ...
        if input_type == 'path':
            f = open(lasso_input)
            lasso_str = f.read().strip()
            f.close()
        if input_type == 'file':
            lasso_str = lasso_input.read().strip()
        if input_type == 'file_str':
            lasso_str = lasso_input.strip()

        # lasso residues
        residues = []
        resi_lines = [] # data holder
        lines = PDB_line.fromlines(lasso_str) # Note LF is required
        for pdb_l in lines:
            if pdb_l.line_type == 'ATOM' or pdb_l.line_type == 'HETATM':
                # Deal with the first residue
                if len(residues) == 0 and len(resi_lines) == 0:
    ...
    def get_lasso_seq(self, Oneletter = 0):
        '''
        get chain sequence from the "residues" list. Store in self.seq
        ----------------------------
        Oneletter
        ...
        * Need to be update after mutation
        ...
        for resi in self.residues:
            ...
            if len(lasso_seq) == 0:
                last_id = resi.id
                lasso_seq.append(resi.name)
                continue
            # missing sequence
            missing_length = resi.id - last_id - 1
            if missing_length > 0:
                lasso_seq = lasso_seq + ['NAN',] * missing_length
...
#------------------------
    @classmethod
    def topology_sort(cls, Lasso_obj):
        '''
        sort the sequence of the lasso peptide into sections
        representative of its topology.
        ranges are set for accomodating various lasso structures
        from scaffold library.
        '''
        upperRange=range(7)
        plugRange=range(7,9)
     ...
     #   self.plugs = list(set(self.plugs))
        ...
        return cls(Lasso_obj.upperLoop, Lasso_obj.tail, Lasso_obj.plugs, Lasso_obj.ring, Lasso_obj.upperPlug, Lasso_obj.lowerPlug, Lasso_obj.essentialRes)
...
test = lassoPeptide.fromPDB('7mr_lassoPeptide.pdb')
lassoPeptide.prin(test)
