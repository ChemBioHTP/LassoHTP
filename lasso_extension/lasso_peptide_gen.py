...
def convert_to_PDB(xyz_str: str, outfile_name: str) -> None:
    ...
    extended.write("pdb", f"{module_file_dir}/output_structures/{outfile_name}.pdb", overwrite=True)
...
def clean_PDB(outfile_name: str, scaffold_file : str, extender_file : str) -> None:
    ...
    """
    with open(f"{module_file_dir}/output_structures/{outfile_name}.pdb", "r", encoding="utf-8") as pdb:
    ...
    with open(f"{module_file_dir}/output_structures/{outfile_name}.pdb", "w", encoding="utf-8") as pdb:
...
def lasso_extender(scaffold_file: str,
                   tail_length: int,
                   outfile: str,
                   extension_file: str = f"{os.path.split(os.path.realpath(__file__))[0]}/extenders/alanine_planar_N.xyz") -> None:
...
def lasso_peptide_gen(ring: int,
                      ...
                      isopeptide: str,
                      outfile: str,
                      extender: str = f"{module_file_dir}/extenders/alanine_planar_N.xyz"):
    ...
    scaffold = f"{module_file_dir}/scaffolds/{ring}{isopeptide}_{loop}.pdb" # this require scaffold is under the same dir as this file
